...
일부 KTF의 기존 저장 기록이 내장 데이터를 없는 것으로 표시하면, 설정·진행 저장을 유지한 채 데이터 길이 정보만 복원합니다.
다른 인증 방식이나 누락된 게임 데이터는 해결하지 못합니다.
일부 KTF의 확인된 실행 파일에는 작은 맵에서 화면이 흔들리는 문제를 막는 호환 보정도 자동 적용됩니다. 원본 파일과 저장 형식은 유지합니다.
...
키패드 배치는 설정의 **🎮 키패드 타입**에서 고릅니다. `Type1`은 방향키·OK와 전체 숫자패드, `Type2`는 숫자키 방향패드가 있는 배치이며, `Type3`은 변형 숫자 배치, `Type4`는 빈 배치입니다. Type1·Type2의 기본 배치가 서로 바뀌었으며, 저장된 커스텀 배치·크기·현재 선택 타입은 유지합니다. **🎮 키패드 세부 조정**에서 크기와 각 칸의 키를 바꿀 수 있습니다.
**연사** 스위치는 Type1·Type2·Type3의 Menu와 Call 사이에 기본으로 있습니다. 저장된 사용자 배치는 유지하며, **🎮 키패드 세부 조정**에서 직접 추가하거나 **기본값으로 되돌리기**로 새 기본 배치를 적용할 수 있습니다. Type4는 빈 배치를 유지합니다. 누를 때마다 `off → manual → auto → off`로 바뀝니다. `manual`은 숫자 5 또는 OK를 누르는 동안 해당 키를 초당 10회 연사하며 키보드에도 적용됩니다. `auto`에서는 숫자 5 또는 OK를 한 번 누르면 손을 떼도 해당 키를 연사합니다. 같은 키를 다시 누르면 멈추고, 다른 대상 키를 누르면 기존 연사를 끄고 그 키로 전환합니다. auto 모드 선택만으로는 연사가 시작되지 않습니다. 게임 실행·복귀, 화면 이탈, 키패드 편집, 문자 입력창 진입 시 `off`로 초기화됩니다. 스위치 배치는 저장하지만 모드는 저장하지 않습니다. 브라우저가 바쁘면 연사 속도가 낮아질 수 있습니다.
