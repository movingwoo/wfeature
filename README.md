...
자바 게임에서는 문자 입력칸을 선택한 뒤 설정의 **문자 입력**을 누르세요. SGS 스크립트가 기본 문자 입력 대화상자를 요청하면 대화상자가 자동으로 표시됩니다. 그 안의 입력칸을 누른 뒤 폰·PC 기본 키보드로 입력하고, 한글 조합을 마친 다음 **입력**을 누르면 게임에 반영됩니다. SGS 대화상자의 **취소**는 기존 값을 유지한 채 게임에 입력 종료를 알립니다. KTF·SKT의 지원되는 입력칸에서 사용할 수 있으며, 게임이 직접 그린 입력창과 LGT 입력창은 아직 지원하지 않습니다.

SGS 게임이 외부 웹 주소를 요청하면 화면 위에 주소와 링크가 표시됩니다. 주소는 자동으로 열리지 않으며, 표시된 링크를 직접 눌러야 새 탭에서 열립니다. **닫기**는 알림만 치우고 게임 실행에는 다른 동작을 전달하지 않습니다. CLI 진단 실행에서는 외부 주소 열기를 지원하지 않는다고 명시적으로 알립니다.
