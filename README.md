...
이렇게 넣은 파일은 `var/ext/`에 복사되며 원본 파일은 유지됩니다.  
**여기 들어간 게임만 화면의 게임 삭제 버튼으로 지울 수 있습니다.**  
...
게임은 서버 전체에서 실행·대기 합계 최대 4개를 유지합니다.  

**세이브 내보내기 / 세이브 가져오기**로 세이브를 파일 하나로 주고받을 수 있습니다.  

## 조작

키보드와 화면 키패드를 같이 쓸 수 있습니다.
게임에서 문자 입력칸을 선택한 뒤 **Opts → 🔤 문자 입력**을 누르면 폰·PC 기본 쿼티 키보드와 한글 IME로 입력할 수 있습니다. 창을 열면 초록색 **입력 가능** 또는 빨간색 **입력 불가능**이 표시됩니다. 한글 조합을 마치고 **입력**을 누르면 게임에 반영됩니다. 기존 글자 뒤에 추가하는 입력칸에서는 **삽입**으로 표시됩니다.
...
- **🎵 배경음 / 🔊 효과음** — 배경음과 효과음을 조절하며 브라우저별로 기억합니다.
...
- **📳 진동** — 게임이 요청한 진동을 실제로 울릴지 정합니다. 진동을 지원하지
  않는 브라우저에서는 나오지 않습니다.
- **⏩ 속도** — 0.25배에서 4배까지 속도를 조절합니다.  
- **⌨️ 키보드 매핑 변경** — 키보드 키를 튜닝합니다.  
- **🎯 치트** — 별도 설명을 참조해주세요.
