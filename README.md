...
게임에서 문자 입력칸을 선택한 뒤 **Opts → 문자 입력**을 누르면 폰·PC 기본 쿼티 키보드와 한글 IME로 입력할 수 있습니다. 한글 조합을 마치고 **입력**을 누르면 게임에 반영됩니다. KTF·LGT·SKT에서 지원하며 별도 천지인 키패드는 사용하지 않습니다. SKT의 게임 소유 입력칸과 LGT의 WIPI-C 입력창은 현재 값을 읽을 수 없어 작성한 내용을 게임 커서 위치에 추가합니다. 이 경로는 한글·영문·숫자·기본 기호를 지원하며, 글자 단위 전달에서 깨지는 이모지 등 보조 평면 문자는 거절합니다. 숫자 모드에서는 숫자만 입력할 수 있고 LGT 문자 모드는 EUC-KR 범위로 제한됩니다. 별도 IME 리스너가 설치된 KTF·LGT Java 입력칸 등 게임 고유 계약을 확인할 수 없는 입력창은 지원하지 않습니다.
